...
export default function AdminInventarioTransferenciasNuevoPage() {
  ...
  const [transferenciasExpandidas, setTransferenciasExpandidas] = useState<Record<string, boolean>>({})
  ...
  const almacenDestinoIdUrl = searchParams.get("almacen_destino_id")
  ...
  async function cargarTodo() {
    ...
    const mapaAlmacenes = new Map<string, { nombre: string }>()
    ...
    const itemsMap = new Map<string, TransferenciaItemDB[]>()
    ...
    const central = almacenesData.find((a) => (a.tipo || "").toLowerCase() === "central")

    if (almacenDestinoIdUrl && almacenesData.some((a) => a.id === almacenDestinoIdUrl)) {
      setAlmacenDestino(almacenDestinoIdUrl)
    }
    ...
    if (central?.id) {
      setAlmacenOrigen((prev) => prev || central.id)
    } else if (almacenesData.length > 0) {
      setAlmacenOrigen((prev) => prev || almacenesData[0].id)
    }
  ...
  function limpiarItemActual() {
    setProductoId(productoIdUrl && productos.some((p) => p.id === productoIdUrl) ? productoIdUrl : "")
    setCantidad("")
  }
  ...
  function toggleExpandirTransferencia(id: string) {
    setTransferenciasExpandidas((prev) => ({
      ...prev,
      [id]: !prev[id],
    }))
  }
  ...
  async function guardarTransferenciaCompleta() {
    ...
    if (productoIdUrl && productos.some((p) => p.id === productoIdUrl)) {
      setProductoId(productoIdUrl)
    } else {
      setProductoId("")
    }
  ...
  const contadoresEstado = useMemo(() => {
    const pendientes = transferencias.filter((t) => normalizarEstado(t.estado) === "pendiente").length
    const confirmadas = transferencias.filter((t) => normalizarEstado(t.estado) === "confirmada").length
    const canceladas = transferencias.filter((t) => normalizarEstado(t.estado) === "cancelada").length
  ...
  const transferenciasFiltradas = useMemo(() => {
    if (filtroEstado === "todos") return transferencias
    return transferencias.filter((t) => normalizarEstado(t.estado) === filtroEstado)
  }, [transferencias, filtroEstado])
  ...
  return (
    <div className="min-h-screen bg-[#0F6D6A] p-6 md:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {(productoIdUrl || almacenDestinoIdUrl) && (
          <div className="bg-teal-50 border border-teal-200 rounded-2xl p-4">
            <p className="text-sm text-teal-700 font-semibold">
              Transferencia iniciada desde alerta de inventario
            </p>
            <p className="text-sm text-teal-800 mt-1">
              {productoIdUrl ? "Producto preseleccionado. " : ""}
              {almacenDestinoIdUrl ? "Almacén destino preseleccionado." : ""}
            </p>
          </div>
        )}
            ...
            <div className="bg-white rounded-3xl shadow-2xl p-6">
              <div className="flex flex-col gap-4 mb-5">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                  ...
                  <div className="bg-[#0F6D6A]/10 text-[#0F6D6A] text-sm font-bold px-4 py-2 rounded-full">
                    {transferenciasFiltradas.length} visibles
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-3">
                  <button
                    ...
                    className={`px-4 py-2 rounded-2xl text-sm font-bold transition border ${
                      filtroEstado === "todos"
                        ? "bg-[#0F6D6A] text-white border-[#0F6D6A]"
                        : "bg-white text-[#0F6D6A] border-[#0F6D6A]/20 hover:bg-[#0F6D6A]/5"
                    }`}
                  >
                    Todas ({contadoresEstado.todos})
                  </button>
                  ...
                  >
                    Pendientes ({contadoresEstado.pendiente})
                  </button>
                  ...
                  >
                    Confirmadas ({contadoresEstado.confirmada})
                  </button>
                  ...
                  >
                    Canceladas ({contadoresEstado.cancelada})
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-4">
                {transferenciasFiltradas.map((t) => {
                  const items = t.items || []
                  const expandida = Boolean(transferenciasExpandidas[t.id])
                  const itemsVisibles = expandida ? items : items.slice(0, 4)
                  const restantes = Math.max(items.length - 4, 0)
                        ...
                        <div className="space-y-3 flex-1 min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span
                              className={`px-3 py-1 rounded-full text-xs font-bold ${
                                normalizarEstado(t.estado) === "confirmada"
                                  ? "bg-green-100 text-green-700"
                                  : normalizarEstado(t.estado) === "cancelada"
                          ...
                          <div className="space-y-2">
                            ...
                            {restantes > 0 && !expandida && (
                              <button
                                type="button"
                                onClick={() => toggleExpandirTransferencia(t.id)}
                                className="text-sm font-bold text-[#0F6D6A] hover:text-[#0a5654] px-2 py-1"
                              >
                                Ver {restantes} producto(s) más
                              </button>
                            )}

                            {items.length > 4 && expandida && (
                              <button
                                type="button"
                                onClick={() => toggleExpandirTransferencia(t.id)}
                                className="text-sm font-bold text-[#0F6D6A] hover:text-[#0a5654] px-2 py-1"
                        ...
                        <div className="lg:min-w-[220px] space-y-3">
                          ...
                          <div className="flex flex-col gap-2">
                            <Link
                              ...
                              className="bg-[#0F6D6A] text-white px-4 py-2 rounded-xl font-bold text-center hover:bg-[#0a5654] transition"
                            >
                              Ver detalle
                            </Link>

                            {normalizarEstado(t.estado) === "pendiente" && (
                              <Link
                                href={`/admin/inventario/transferencias-nuevo/${t.id}/editar`}
                                className="bg-[#F47C3C] text-white px-4 py-2 rounded-xl font-bold text-center hover:bg-[#db6d31] transition"
                              >
                ...
                {transferenciasFiltradas.length === 0 && (
                  <div className="rounded-3xl border border-dashed border-gray-300 p-10 text-center">
                    ...
                    <p className="text-gray-500 mt-2">
                      Cambie el filtro o cree una nueva transferencia.
                    </p>
                  </div>
                )}
              </div>
